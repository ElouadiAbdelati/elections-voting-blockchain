...
let Candidat = require('./Candidat.js');
...
class VoteContract extends Contract {
  ...
  async init(ctx) {
    ...
    //create voters
    let voter1 = await new Voter('V1', 1);
    let voter2 = await new Voter('V2', 2);
    ...
    if (currElections.length === 0) {
      ...
      //create the election
      election = await new Election('barnaman',2021, electionStartDate, electionEndDate);
    ...
    //create votableItems for the ballots  candidatId, description, nomSGP, prenomSGP
    let repVotable = await new Candidat(ctx, '123456',' description','abdelati','elouadi',1,1);
    let demVotable = await new Candidat(ctx, '978545', ' description','abdel','el',2,2);
    
    //populate choices array so that the ballots can have all of these choices 
    candidats.push(repVotable);
    candidats.push(demVotable);
    

    for (let i = 0; i < candidats.length; i++) {
    
      //save votable choices in world state
      await ctx.stub.putState(candidats[i].candidatId, Buffer.from(JSON.stringify(candidats[i])));
    ...
    for (let i = 0; i < voters.length; i++) {
      ...
      if (!voters[i].ballot) {
        ...
        await this.generateBallot(ctx, candidats, election, voters[i]);
   ...
   */
  async generateBallot(ctx, candidats, election, voter) {

    //generate ballot
    let ballot = await new Ballot(ctx, candidats, election, voter.voterId);
  ...
  async createVoter(ctx, args) {
    ...
    //create a new voter
    let newVoter = await new Voter(args.voterId, args.regionId);
    ...
    let candidats = JSON.parse(await this.queryByObjectType(ctx, 'candidatItem'));
    
    //generate ballot with the given votableItems
    await this.generateBallot(ctx, candidats, currElection, newVoter);
